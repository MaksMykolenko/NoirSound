    ...
    <div className="space-y-4" data-testid="lyrics-editor">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs leading-relaxed text-zinc-500">{t('upload.lyricsHelp')}</p>
        </div>
        <button
          type="button"
          className="ns-button-secondary inline-flex items-center gap-2 px-3 py-2 text-xs"
      ...
      {preview ? (
        ...
        <label className="block space-y-1.5">
          ...
          <textarea
            id={`${idPrefix}-text`}
            rows={compact ? 10 : 14}
            className="ns-field min-h-56 resize-y px-4 py-3 font-sans text-sm leading-7"
            value={text}
            onChange={(event) => {
      ...
      <div className="rounded-2xl border border-zinc-800 bg-zinc-950/40 p-4 space-y-3">
        ...
        {hasLyrics && !value.lyricsRightsConfirmed && (
          <p className="text-xs font-semibold text-amber-300" role="alert">{t('lyrics.rightsRequired')}</p>
        )}
      </div>
      ...
      {hasLyrics && (
        <button
          type="button"
          className="inline-flex min-h-11 items-center gap-2 text-xs font-bold text-rose-300 hover:text-rose-200"
          onClick={() => onChange({
            ...value,
            lyricsText: '',
            lyricsType: 'NONE',
